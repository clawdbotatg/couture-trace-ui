...
// ─── Default View Model ──────────────────────────────────────────────────────
// This is the shape that <TermCanvas vm={...} /> expects.
// All panels read from this single object — add fields as needed.
//
const INIT = {
  // Header
  pc: 6, acc: 2, zero: false, carry: false,
  viewer: 'Atelier',
  nextInstruction: 'L2 STORE 1',
  mood: 'lifting values into the spotlight',
  layersActive: 3,

  // Stage Monitor
  status: 'RUNNING', theme: 'Velvet', stepMs: 30,
  progress: 34, throughput: 28,
  memoryCells: 5, traceEvents: 25, proof: 'READY',
...
// ─── Simulator ───────────────────────────────────────────────────────────────
// Drives fake VM activity for demo purposes.
// In a real app, replace this with your actual data source (WebSocket, etc).
//
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setVm(prev => {
        ...
        const layer  = LYRS[Math.floor(Math.random() * LYRS.length)]
        const newEntry = {
          num:    prev.traceEvents + 1,
          layer, op,
        ...
        return {
          ...
          pressure: Math.max(0, Math.min(100, prev.pressure + Math.floor(Math.random() * 7) - 3)),
        }
      })
    }, 600)
